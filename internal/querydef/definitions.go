...
var builtinDimensions = []BuiltinDimension{
	DimensionClient, DimensionModel, DimensionProvider, DimensionProject, DimensionDay, DimensionMonth,
}

// BuiltinDimensionNames 返回内置维度名的规范顺序独立副本(client、model、
// provider、project、day、month):TUI 候选列表等调用方的唯一名单来源,调用方不得
...
// 八个内置视图名与 custom/list 两个固定入口名。
var reservedNameOrder = []string{
	"client", "model", "provider", "project", "session", "summary", "day", "month", "custom", "list",
}
