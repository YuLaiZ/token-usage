...
func (p *dataDirPage) title() string { return "数据目录(只读)" }
...
func (p *dataDirPage) View() string {
	s := "数据目录(只读)\n\n"
	s += "  当前 data_dir: " + p.app.display.DataDir + "\n\n"
	s += "配置文件路径(固定):\n"
	s += "  ~/.token-usage/config.toml\n"
	s += "  (由 config.ConfigPath(home) 统一计算,TUI/CLI/configapp 共用)\n\n"
	s += "data_dir 在 TUI 只读,无法在此页直接修改。原因:\n"
	s += "  - data_dir 变更需要迁移 usage.db 与 logs/ 目录\n"
	s += "  - 迁移前必须先停止守护进程,避免写入冲突与数据丢失\n"
	s += "  - 迁移失败可能导致历史用量数据不可用\n\n"
	s += "如需修改 data_dir,请在命令行执行(迁移需 --confirm-migrate 二次确认):\n"
