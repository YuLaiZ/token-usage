...
// 候选集合严格受限:custom 只选内置维度(名单来自 querydef 单一来源);
// group 只选 builtin+已定义 custom;default 只选 builtin+custom+group。
func TestQueryViews_CandidateSetsRestricted(t *testing.T) {
	...
	if len(sub.sel.candidates) != 6 {
		t.Errorf("custom 候选应恰为 6 个内置维度: %v", sub.sel.candidates)
	}
	for _, c := range sub.sel.candidates {
		if c != "client" && c != "model" && c != "provider" && c != "project" && c != "day" && c != "month" {
			t.Errorf("custom 候选含非法项 %q", c)
		}
	}

	grp := newGroupEditPage(a, "g2", nil)
	want := []string{"client", "model", "provider", "project", "day", "month", "mpc"}
	...
	if len(def.items) != 9 { // 6 builtin + mpc + g + 使用默认 client
...
func TestQueryViews_GroupEditAndDefaultSelect(t *testing.T) {
	...
	// group:选 client、mpc(候选序 client model provider project day month mpc)。
	g := newGroupEditPage(a, "g", nil)
	for _, key := range []string{" ", "down", "down", "down", "down", "down", "down", " ", "enter"} {
