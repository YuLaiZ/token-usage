...
func newLogPage(app *App) *logPage {
	...
	dir.Placeholder = "<data_dir>/logs (默认)"
	max := textinput.New()
	max.SetValue(strconv.Itoa(app.draft.Log.MaxDays))
	max.Placeholder = "7 (默认)"
	return &logPage{
		app:          app,
...
func (p *logPage) title() string { return "日志" }
...
func (p *logPage) commit() error {
	...
	if err != nil {
		p.feedback = "max_days 必须是整数(0 表示使用默认值)"
		return fmt.Errorf("log.max_days %q 非法: %w", p.maxDaysInput.Value(), err)
	}
	if n < 0 {
		p.feedback = "max_days 不能为负数(0 表示使用默认值)"
		return fmt.Errorf("log.max_days 不能为负数(当前 %d)", n)
	}
	if !isRegisteredLogLevel(p.level) {
		p.feedback = "未知的日志级别(空格切换可选: 默认/info/debug/warn/error)"
		return fmt.Errorf("log.level %q 未注册(受支持: %v)", p.level, runtimecfg.RegisteredLogLevels())
	}
	// 校验通过:整体写入,清空错误反馈。
