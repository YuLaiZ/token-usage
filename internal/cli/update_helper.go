...
var errHelperNotSupported = errors.New("自更新 helper 命令仅在 Windows 平台受支持")
...
func cleanupHelperIdentity(helperPID int, helperCreationTime uint64) (update.ProcessIdentity, error) {
	if helperPID <= 0 || uint64(helperPID) > uint64(^uint32(0)) {
		return update.ProcessIdentity{}, fmt.Errorf("cleanup 缺少合法 helper PID: %d", helperPID)
	}
	...
	if !identity.Valid() {
		return update.ProcessIdentity{}, fmt.Errorf("cleanup 缺少合法 helper 身份（PID=%d CreationTime=%d）", helperPID, helperCreationTime)
	}
	return identity, nil
}
...
func newUpdateHelperCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:    "_update-helper",
		Short:  "内部命令（Windows 自更新后台 helper，不直接调用）",
		Hidden: true,
		Args:   cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			if planPath == "" {
				return fmt.Errorf("缺少 --plan 参数")
			}
			return runUpdateHelperCmd(cmd.Context(), planPath)
		},
	}
	cmd.Flags().StringVar(&planPath, "plan", "", "helper 计划文件路径")
	return cmd
}
...
func newUpdateCleanupCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:    "_update-cleanup",
		Short:  "内部命令（Windows 自更新临时文件清理，不直接调用）",
		Hidden: true,
		Args:   cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			if planPath == "" {
	...
	cmd.Flags().StringVar(&planPath, "plan", "", "helper 计划文件路径")
	cmd.Flags().IntVar(&helperPID, "helper-pid", 0, "待清理的 helper 进程 PID")
	cmd.Flags().Uint64Var(&helperCreationTime, "helper-creation-time", 0, "待清理的 helper 进程创建时间（FILETIME 原始值，与 --helper-pid 配套用于身份校验）")
	return cmd
}
