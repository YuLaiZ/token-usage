...
func newServeCmdWithDeps(load func() (*config.Config, error), open func(string) (*db.DB, error), version string) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: ui.Bi(
			"Start a read-only local HTTP server that serves the built-in dashboard: the embedded HTML page at / with JSON endpoints (/api/meta, /api/dashboard) and SVG charts (/api/chart/{kind}.svg). The server binds to 127.0.0.1 only by default; binding a public address such as 0.0.0.0 exposes your usage data to the local network. The HTTP data surface is strictly read-only — no CORS headers (same-origin use), no database or configuration writes; the only local files involved are the lifecycle state file serve.json (shared by the foreground and background forms) and serve.log for background runs. At most one dashboard instance (foreground or background) runs at a time: a second serve — in either form, regardless of the address — is rejected by a single-instance guard before listening, printing the running instance's URL and PID and exiting 0 idempotently (stop it first with `token-usage serve stop`); the serving process holds a serve.lock lifecycle lock in the data directory for its whole lifetime. Press Ctrl+C to stop. For unattended use, `serve start` runs the same dashboard in the background and `serve status` / `serve stop` inspect and stop it.\n\nExamples:\n  token-usage serve\n  token-usage serve --open\n  token-usage serve --addr 127.0.0.1:9000\n  token-usage serve start\n  token-usage serve status\n  token-usage serve stop",
			"启动只读的本地 HTTP 服务，提供内嵌仪表板页面（/）及其 JSON 接口（/api/meta、/api/dashboard）与 SVG 图表（/api/chart/{kind}.svg）。默认仅绑定 127.0.0.1；绑定 0.0.0.0 等公网地址会把统计数据暴露给局域网。HTTP 数据面严格只读：不设 CORS 头（按同源使用）、不写数据库与配置；本地仅涉及服务生命周期状态文件 serve.json（前台/后台共用）与后台日志 serve.log。任意时刻至多一个仪表板实例（前台或后台）在运行：第二个 serve——无论前台后台、无论请求哪个地址——都会在监听之前被单实例守卫拒绝，打印运行中实例的 URL 与 PID 并以退出码 0 幂等返回（要重启请先用 `token-usage serve stop` 停止）；服务主体在其整个生命周期持有数据目录下的 serve.lock 生命周期锁。按 Ctrl+C 停止。无人值守场景可用 `serve start` 把同一仪表板转入后台运行，`serve status` / `serve stop` 查看与停止。\n\n示例：\n  token-usage serve\n  token-usage serve --open\n  token-usage serve --addr 127.0.0.1:9000\n  token-usage serve start\n  token-usage serve status\n  token-usage serve stop",
		),
		Args: cobra.NoArgs,
