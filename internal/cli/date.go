...
func parseDateArgs(args []string, defaultToday bool, cmdName string) ([]string, error) {
	...
	if len(args) > 1 {
		return nil, fmt.Errorf("无效的日期参数：仅接受 0 或 1 个位置参数，得到 %d 个。接受 YYYYMMDD 或 YYYYMMDD-YYYYMMDD，例如 token-usage %s 20260701", len(args), cmdName)
	}
	...
	if strings.Contains(raw, "-") {
		...
		if end.Before(start) {
			return nil, fmt.Errorf("无效的日期参数 %q：结束日期不能早于开始日期。接受 YYYYMMDD 或 YYYYMMDD-YYYYMMDD，例如 token-usage %s 20260701", raw, cmdName)
		}
		var dates []string
	...
	if err != nil {
		return nil, fmt.Errorf("无效的日期参数 %q：接受 YYYYMMDD 或 YYYYMMDD-YYYYMMDD，例如 token-usage %s 20260701", raw, cmdName)
	}
...
func parseErrorDateArg(args []string) (string, error) {
	...
	if len(args) > 1 {
		return "", fmt.Errorf("无效的日期参数：仅接受 0 或 1 个位置参数，得到 %d 个。只接受 YYYYMMDD（8 位），例如 token-usage errors 20260701", len(args))
	}
	raw := args[0]
	// 拒绝范围与含连字符的形式。
	if strings.Contains(raw, "-") {
		return "", fmt.Errorf("无效的日期参数 %q：只接受 YYYYMMDD（8 位），例如 token-usage errors 20260701", raw)
	}
	t, err := parseCompactDate(raw)
	if err != nil {
		return "", fmt.Errorf("无效的日期参数 %q：只接受 YYYYMMDD（8 位），例如 token-usage errors 20260701", raw)
...
func parseCompactDate(s string) (time.Time, error) {
	if len(s) != 8 {
		return time.Time{}, fmt.Errorf("日期长度应为 8 位 YYYYMMDD: %q", s)
	}
...
func parseRangeEndpoints(startStr, endStr, raw, cmdName string) (time.Time, time.Time, error) {
	if len(startStr) != 8 || len(endStr) != 8 {
		return time.Time{}, time.Time{}, fmt.Errorf("无效的日期参数 %q：区间端点应为 8 位 YYYYMMDD。接受 YYYYMMDD 或 YYYYMMDD-YYYYMMDD，例如 token-usage %s 20260701", raw, cmdName)
	}
	start, err := time.Parse("20060102", startStr)
	if err != nil {
		return time.Time{}, time.Time{}, fmt.Errorf("无效的日期参数 %q：开始日期不合法。接受 YYYYMMDD 或 YYYYMMDD-YYYYMMDD，例如 token-usage %s 20260701", raw, cmdName)
	}
	end, err := time.Parse("20060102", endStr)
	if err != nil {
		return time.Time{}, time.Time{}, fmt.Errorf("无效的日期参数 %q：结束日期不合法。接受 YYYYMMDD 或 YYYYMMDD-YYYYMMDD，例如 token-usage %s 20260701", raw, cmdName)
	}
