...
每条接受日期参数的 query 命令的输出以一个双语统计信息区开始（多表组合也只打印一次）：统计范围、K/M/B 数值单位换算说明（`单位`）、范围内最新消息事件时间（`数据截至`）与最近一次成功采集完成时间（`最近成功采集`）；无数据的项显示 `—`。`query list` 只读配置，没有统计信息区。各字段的确切含义见 **[CLI 参考](docs/cli.zh-CN.md)**。
