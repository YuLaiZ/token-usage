...
- Collectors for Claude Code/Desktop, OpenCode, Codex, WorkBuddy, ZCode, Zhipu-AutoClaw, and MiMo Code + MiMo Desktop (two clients attributed from one shared local database via session version markers).
...
`update` prints its progress step by step — version check, current/target version, download, verify, install, daemon and dashboard switch — and on an interactive terminal the download shows a live single-line indicator (percentage, transferred/total bytes, average speed); redirected output keeps only the step lines. A daemon that was running before the update is stopped and restarted automatically on the new binary; a daemon that was stopped stays stopped (the success output points to `token-usage daemon start`). The local dashboard is preserved the same way: a dashboard that answers on `/api/meta` before the update is stopped for the replacement and then restored in the background at its original address with the new binary (the browser is never opened automatically); a stopped one is not started. A missing, corrupt, or stale `serve.json` counts as not running and is left untouched, a dashboard that cannot be stopped aborts the update before the binary is replaced, and any failure rolls the update back to the old binary and the pre-update daemon/dashboard state.
...
token-usage query weekday     # usage by weekday
