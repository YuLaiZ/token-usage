...
| `internal/querydef/` | Pure-function parser that turns the raw query state into validated, read-only definitions (built-in dimensions, subqueries, groups, default, plus the `[query.output]` column layout); it exposes one full `Parse` and two isolated read entries (`ParseViews` for view definitions, `ParseOutputLayout` for the output layout) that ignore each other's errors and share the top-level-issue precondition; errors are structured `ValidationError` diagnostics (stable path + closed kind set). No file or DB access, and `internal/config` does not depend on it. |
...
| MiMo Code + MiMo Desktop (one shared database) | SQLite; sessions are attributed per product via `session.version` (strict `desktop-<hash>` → MiMo Desktop, everything else → MiMo Code) | `~/.local/share/mimocode/mimocode.db` |
...
- The query layer (`querier`) directly joins `messages` (plus `sessions` metadata) and aggregates in real time; there is no intermediate summary table. All grouped views share one dimension-based pipeline (`dimensions → raw aggregates → alias-merged, case-folded composite keys → stable sort → table`) and end with a `Total / 总计` row aggregated separately over the same date range; session details and the summary are exempt. Grouping keys are case-insensitive (Unicode simple case folding, the same equivalence `strings.EqualFold` uses): case-only variants of a value merge into one row, whose representative spelling is the variant with the most requests (ties break to the smallest spelling by byte order). Provider aliases merge rows before composite keys are formed, with a case-insensitive (`EqualFold`) fallback lookup when no exact key matches, without rewriting `messages`. Every table renders its metric columns through one ordered set of metric descriptors resolved from the global `[query.output]` layout (default seven columns; `cache_create` selectable but hidden) — headers, group rows, total rows, and session rows all walk the same descriptors, and `cache_hit` always reads the full aggregate including `cache_create`, so the layout changes display only, never statistics, sorting, or totals.
...
The mimocode source produces two canonical clients: `MiMo Code` (CLI) and `MiMo Desktop` (the desktop app), mirroring Claude Code/Claude Desktop under the single `mimocode` config key (config.toml, CLI flags, and the config TUI — the config UI never shows the client names). Attribution is decided per session from `session.version` in the shared SQLite database (`model.MiMoSessionClient`): a strict `desktop-<hex-hash>` version belongs to `MiMo Desktop`; every other form — `0.x` CLI versions, the `2.1.x` series, empty or unseen version shapes — belongs to `MiMo Code`; provider/model never participate. After a v5 upgrade an automatic, resumable, lossless reconciliation re-keys existing rows by these rules on the next mimocode collection, including daemon startup catch-up (batched, each batch a single transaction: merge opposite-client history → upsert current source values → delete merged source rows → advance the pending cursor → commit; sessions that no longer exist in the source database keep their stored identity untouched). Every later mimocode write batch also builds a target client per touched session, merges all opposite-client history before writing current values, then deletes the source side in the same transaction; the split-trigger bypass protects the Desktop-to-Code direction. This makes session-version changes safe in both directions without duplicate `(client,id)` rows or token double counting. An explicit `collect all --client mimocode` re-arms pending and reruns the complete assignment reconciliation, including sessions with no remaining source messages. A query may install the migration but does not read the MiMo source database, so it leaves reconciliation pending. Rollback compatibility works through the trigger chain legacy name → `MiMo Code` → `MiMo Desktop`, with legacy writes re-arming the pending marker so a rolled-back period self-heals on the next mimocode collection. The legacy long name survives only as `model.LegacyClientXiaomiMiMoCode` for the migrations, the compatibility triggers, and a defensive display fallback — it is not a current client name.
