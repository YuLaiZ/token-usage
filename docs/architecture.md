...
Defaults work out of the box: a client only needs `enabled = true`; the `runtimecfg` registry fills data-source paths from each tool's default location. Use a dotted key in the same section style to override a default for personalization.
