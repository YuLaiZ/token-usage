...
Dimension grouping is case-insensitive in every grouped view (Unicode simple case folding — the same equivalence `strings.EqualFold` uses). Values that differ only by case merge into one row; the merged row shows the spelling with the most requests in the queried range (ties break to the smallest spelling by byte order), and its aggregates are the sums of the variants. This covers the text dimensions (`client`, `model`, `provider`, `project`) as well as the time dimensions (whose grouping values never differ by case, so their behavior is unchanged) in built-in views, custom views, and the serve dashboard alike; `messages` source data is never rewritten.
...
`query summary` renders a fixed vertical summary: `Clients / 客户端数` (distinct clients counted under the same case-insensitive folding as the client grouping), `Total requests / 请求总数`, `Active days / 活跃天数` (days with data inside the range), one line per token column (`Input` through `Total`, always including `Cache Create`), and, when the range contains at least one day with data, `Peak day / 单日峰值` (the date with the highest source total, ties broken by earliest date) and `Daily average / 日均总量` (range total divided by active days, integer division rounded down). Those last two lines are omitted when the range has no data.
...
- `serve status --format json` emits the same outcome as a machine-readable document (two-space indent, trailing newline, the same convention as `doctor --format json` and `daemon status --format json`): `state` uses a closed vocabulary — `running`, `not_running`, `not_running_stale_removed`, `not_running_corrupt_removed` — `running` is the boolean the state derives from, `pid`/`addr`/`url`/`started_at` (RFC3339, verbatim from serve.json) appear only while running, and `data_dir` carries the configured data directory. An invalid `--format` value errors with the allowed set, matching `daemon status`.
...
`update` prints its progress while it works: a `Checking for updates…` line, the current/target version pair as soon as a newer release is found (before source verification, so refusals also show it), then step lines for downloading, verifying, stopping the daemon and the dashboard, installing, and restarting them. On an interactive terminal the download renders a single-line live indicator (percentage, transferred/total bytes, average speed); when stdout is redirected or piped the indicator is omitted and only the step lines are printed, and a failed download always closes the line cleanly. A daemon that was running before the update is stopped and restarted automatically on the new binary. The dashboard is preserved the same way: a running dashboard (one that answers on `/api/meta`) is stopped before the replacement — on Windows this also releases the old `.exe` for the background helper — and restored after it as a background instance at its original address with the new binary; the automatic restore never opens a browser. Only an answering dashboard counts as running: a missing, corrupt, or stale `serve.json` is treated as not running and left untouched. If the dashboard cannot be stopped, the update aborts with the binary unreplaced; if any later step fails, the update rolls back to the old binary and the pre-update daemon/dashboard state, keeping both the primary and the rollback failure messages. `update --check` prints only the checking line and its result; it never reads, stops, starts, or cleans up daemon or dashboard runtime state.
...
| Form | Purpose |
|------|---------|
| `update` | Updates to the latest stable Release. If a restricted transaction journal from an interrupted POSIX update exists beside this binary, it is recovered first (the journal also records whether the dashboard was running and its address, so recovery restores it too); a new replacement then proceeds only when the target is strictly higher than the current version and the current source is trusted. It downloads the asset, verifies its SHA256 against the `SHA256SUMS` manifest, stages a `--version` second check, and replaces the binary. A daemon that was running before the update is restarted automatically on the new binary; a daemon that was stopped stays stopped, and the success output points to `token-usage daemon start`. A dashboard that was running before the update is likewise restored in the background at its original address with the new binary; the update's lifecycle actions reuse the same internal orchestration as `serve stop`/`serve start` (no CLI subprocess, no shell), the restore never opens a browser, and on Windows the stop also releases the old `.exe` so the background helper can complete the replacement. |
